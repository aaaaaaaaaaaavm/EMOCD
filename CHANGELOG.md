...
| ID | Item | Detail |
|---|---|---|
| A5-01 | **A5 GMAT complete — verdict `FAIL`** | The low-activity leg finished: baseline 2359.1 d, boosted 4892.4 d, **multiplier 2.0739, +15.21 % against ×1.80**, outside the ±5 % band. With high (1.7302) and mean (1.7750), the **invariance spread is 18.48 % against a ≤5 % band declared before the run.** The point value survives at mean and high activity; the *invariance* does not. `validation/results/A5_astro.json` regenerated and carries the FAIL. |
| A5-02 | **Mechanism identified and tested, not guessed** | `astro.py` models solar activity as a uniform multiplicative scale on density. Sweeping that scale over **40×** (0.25 → 10.0) moves the multiplier only 1.7992 → 1.7968 — constant to 0.1 %. A uniform density factor divides both lifetimes by the same number, so **the ratio is invariant by construction of the model, not as a physical result.** MSIS changes the shape of the density–altitude profile with F10.7, the boosted orbit's apogee sits ~37 km higher, and the ratio then moves 1.73 → 2.07. Reproducible from `analysis/astro.py` `lifetime()` unedited. |
| A5-03 | **`OPEN_PROBLEMS.md` P16 (new, HIGH)** | Records the falsification, the 40× sweep table as evidence, and every location the claim appears — including the **abstract** of `paper/paper.tex` ("invariant across ballistic coefficient and a fivefold solar-activity density range"), Sec. V-B ("invariant to two decimal places") and Limitations, which calls the invariance "the defensible result". E6 updated: it warned the absolutes were weak, and now has to say the ratio is weaker than advertised too. |
| A5-04 | **Absolute-lifetime error changes sign** | GMAT is **2.5× longer** than `astro.py` at low activity, **9 % shorter** at mean, **23 % shorter** at high. E6 predicted absolute disagreement; it did not predict a sign change across the range. Same profile-shape effect, seen from the other side. |
| A5-05 | **Correction to my own text in `VALIDATION_REPORT.md`** | Section 2 read "Invariance across activity: 2.55 % spread, inside the ≤5 % band" and "the claim the paper actually defends survives an independent propagator". Both were computed from two levels before the third finished, and both were wrong. Replaced with the three-level result and the mechanism, and the retraction is stated in place rather than quietly overwritten. |
| DOC-02 | Front pages corrected | `README.md` and `wiki/Home.md` headline rows now read "×1.80 at mean activity — **invariance falsified, see P16**" instead of "invariant across BC and solar activity"; README's validation summary rewritten around the four run analyses. `RESULTS.md` A5 section replaced with the three-level table, a per-activity chart and the 40×-sweep chart; its status bar moved from `████████░░` "within band" to `██████████` **FAIL**. `INVENTORY.md` A32 flagged against P16. |
