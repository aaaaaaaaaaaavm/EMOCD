...
| 3 | Shim **setting** spec | ±0.050 mm | → ±0.66 % thrust |
...
| **RSS** | **0.101** | → **1.33 % thrust** |

Combined with A4's deflection bias (0.0194 mm, always closing) the total gap error is
**0.121 mm — 2.4× the ±0.65 % currently claimed, giving ±1.58 % thrust spread.**
...
|---|---|---|
| Chassis plate 488×140×6 | Drawn **solid**. A4 gives a 17× stress margin, so this is not a structural part — it is a stiffness part carrying dead mass | Pocketing is PII-2. Do not thin uniformly: deflection goes as 1/t³ |
...
| Halbach array position | **G3-D5: not re-centred** after the chassis grew 360 → 488 mm | Array-to-winding position is what K<sub>t</sub> depends on. **Blocking, and it may invalidate K<sub>t</sub> independently of A1** |
...
Dossier §7: Concept → Analysis → **Simulation** → Prototype → Experiment → Repeatability →
**Manufacturability** → Flight Qualification.
