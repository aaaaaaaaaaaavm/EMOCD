...
Method: per-revolution orbit-averaged decay (Gauss tangential equations, quadrature over eccentric anomaly), static exponential atmosphere (Vallado Table 8-4 class, mean activity). **Model limitation: absolute lifetimes swing ~3–10× with solar cycle; the *ratio* is the robust result** (density-scale factors largely cancel).

| Fire alt (km) | Δv (m/s) | Apogee (km) | Lifetime ratio vs circular |
|---|---|---|---|
| 350 | 15 / 25 | 403 / 438 | **1.62–1.68× / 2.15–2.21×** |
| 400 | 15 / 25 | 453 / 489 | **1.59–1.61× / 2.09–2.11×** |
| 450 | 15 / 25 | 504 / 540 | **1.55–1.56× / 2.01–2.03×** |
| 500 | 15 / 25 | 554 / 591 | **1.54× / 1.99×** |

(Sweep over BC = 40/61/90 kg·m⁻²; ratio nearly invariant to BC and altitude.)
**Headline: one 25 m/s shot ≈ 2.0× orbital lifetime, robust across altitude, ballistic coefficient, and drag-model uncertainty.** Absolute example (BC=61): 450 km circular 1.30 yr → 450×540 km 2.64 yr.
...
Halbach: Br=1.30 T (N45SH), 8 mm magnets, λ=48 mm, 4 segments/λ → B₀=0.760 T surface; two-sided effective ⟨B⟩≈0.62 T across a 12 mm winding gap. Active area 0.061 m² (both faces, 0.34×0.09 m) at K=45 kA/m (adiabatic pulse) → **F_max = 1,717 N**.
...
- Peak current 463 A; **bank sag 96→91.6 V (4.6%)** — comfortably inside servo headroom
- Energy drawn 2.47 kJ; payload KE 1.00 kJ; sled KE 1.00 kJ; regen credit (55% of sled KE) → **net 1.92 kJ/shot, 52% end-to-end efficiency**
...
- Authority is abundant: even 80 cm² pole → 30 kN peak at 22.4 m/s, stop in ~6 cm. **Design is therefore taper-limited, not authority-limited: taper pole entry to cap sled decel ≈200 g (protects sintered NdFeB bonding), stop in ~15–25 cm.**
- Fin (0.86 kg Cu) absorbs ~1.0 kJ/shot → **ΔT = 3.0 K/shot**; radiates between shots. Contactless → cycle life not a limiter.
- Ring-spring catches residual ≤1.5 m/s (≈4.5 J — trivial).

## C5 — Conjunction screening (30 days, Kepler + secular J2, 5 s sampling)

Scenario: 12 prograde 25 m/s shots, 20 min apart, from 450 km/51.6° stage. Period difference 55.7 s/orbit → **along-track phase realignment ≈101 orbits ≈ 6.6 days**.
...
Per shot J = 4 kg × 22.4 m/s = 89.6 N·s (satellite momentum only — sled momentum recovered through brake):
...
Torque null via He cold-gas (10 N class, 1.2 m arm, Isp≈160 s): residual thrust-line arm 5–15 cm → **2.4–7.1 g of helium per shot; 29–86 g for the whole campaign.** Cumulative translation folds into disposal targeting.

## C7 — Tip-off error budget (3U, I_t = 0.042 kg·m²)

| Contributor | ΔL (mN·m·s) | Rate |
|---|---|---|
| Residual trim force 10 N × 5 mm CoM × 20 ms | 1.00 | 1.36 °/s |
| Rail-clearance couple in coast (2 N × 10 mm × 50 ms) | 1.00 | 1.36 °/s |
...
- **Battery:** 12 shots × 2.5 kJ = 30 kJ (8.3 Wh) + ~4 h avionics at 60 W = 250 Wh → **~1.7 kg Li-ion**; POEM solar available for recharge in hosted mode.
...
- **Mass:** structure 27 + stator 36 + sled 4 + brake 3 + cassettes 9 + supercap/PPU 12 + thermal 6 + avionics/battery 8 = **~105 kg dry, ~153 kg loaded** (12×4 kg). ESPA Grande allocation 320–465 kg → >2× margin. (Estimates pending CAD; flagged.)

## C9 — Velocity error → orbit error (450 km, 22.4 m/s)
...
3. C4's plate-drag formula is first-order; pole-geometry factor <1 expected — irrelevant to the conclusion given 20× authority margin.
