...
copper, **armature coils aluminium**. Charging 10 → 16 kV gives exit velocity 230 → 321.56 m/s
roughly linearly, at efficiency falling **19.9 % → 14.9 %**, from stored energy **2.70 → 6.91 MJ**.
...
|---|---|---|---|
| Mean acceleration | 321.56 m/s over 3.9 m → **1352 g** | 91 m/s² → **9.3 g** | **145×** |
| Peak | >600 kN on 20 kg → **~3060 g** | 10.7 g | ~286× |
| Energy per shot | **6.91 MJ** | 2.80 kJ | **2470×** |
| Payload interface | Aluminium **armature coils** on the payload | Magnets on a reusable sled | — |

A standard CubeSat qualifies to roughly 14 g quasi-static. Feng's design imposes ~100× that. This is
...
**On "precise velocity control".** Their mechanism is **charging-voltage selection** (10 → 16 kV,
...
One group, three papers, 2022 → 2025. Architecturally the closest thing to this project's cassette
...
| Item | Change |
|---|---|
| `paper/paper.tex` §I | "no published deployment system operates in the tens of m/s" → restricted to flown hardware; Feng and the Harbin line cited; the g-limit argument made quantitative against their 3.9 m barrel |
...
| `docs/adr/003` | Efficiency argument **withdrawn as false**; "cannot command velocity closed-loop" → a claim about absent dispersion evidence. Decision unchanged, now with numbers |
