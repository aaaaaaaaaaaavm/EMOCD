...
1. New magnetics problem → Planar, units mm, **depth 90 mm** (array width), frequency 0.
2. File → Import DXF → `emocd_cross_section.dxf` (tolerance 0.01).
...
5. Belt labels: Copper, circuits A/B/C, series turns = 1, currents for the static shot: **A = +I, B = −I/2, C = −I/2** with I = rated sheet current × pitch → use **1120 A-turns per belt** (140 kA/m × 8 mm) for the rated point; 0 A for the open-circuit field map.
6. Boundary: surround with an air box ≥ 60 mm beyond geometry; apply **A = 0** on the outer boundary. For strict periodicity, replicate the wavelength 3× and read the centre cell.
