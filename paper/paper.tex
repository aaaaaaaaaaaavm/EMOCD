...
Electrical$\rightarrow$payload efficiency & 1--2\% \cite{turman} & 20\% (Sec.~V)\\
...
On cost, an engineering estimate rather than a business case: the recurring hardware is dominated by the magnet set and the SiC drive, both commodity items, so unit cost is comparable to a small-satellite reaction-wheel assembly and far below a propulsive stage. The per-shot energy cost is the striking figure. At 2.80~kJ drawn per ejection the electrical energy is under one watt-hour, met by solar recharge between shots at zero consumable cost, against the propellant a transfer vehicle would expend for the same manoeuvre. A spring deployer has no energy cost but also no programmability; EMOCD buys deterministic per-satellite velocity for less than a watt-hour a shot.
