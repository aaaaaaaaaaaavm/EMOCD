...
Electrical$\rightarrow$payload efficiency & 14.9--19.9\% \cite{feng2025} & 21.2\% (Sec.~V)\\
...
The sled leaves release with about 1.29~kJ. Motor regeneration alone cannot arrest it, because braking force is bounded by the same thrust constant as acceleration; that bound sets what regeneration can and cannot do, and both halves matter. Braking at the rated sheet current over the 240~mm of stator the closed envelope allows downstream of release extracts 339~J of mechanical work for 15~J of copper, returning \textbf{297~J} to the bank, and the sled still reaches the brake at 14.2~m/s carrying \textbf{952~J}. A copper-fin eddy-current brake therefore provides bulk arrest: force proportional to velocity, contactless, with flight heritage in the damper class \cite{eddy}. Authority is abundant, so the pole entry is tapered to cap sled deceleration near 200~$g$. This limit is set by the magnet retention: a 65~g Halbach block at 200~$g$ loads its bonded interface (10.8~cm$^2$) to only 0.12~MPa against a conservative 10~MPa epoxy allowable, a margin of 84, so 200~$g$ is a deliberate conservatism protecting the brittle sinter rather than a strength limit. The 0.86~kg copper fin absorbs the sled's residual 952~J per shot at an adiabatic 2.9~K per-shot transient rise, radiating between shots; a ring-spring catches the residual below 1.5~m/s (Fig.~\ref{fig:brake}). Pulse energy comes from a 6~F, 96~V supercapacitor bank switched by a SiC bridge \cite{faure}. Materials follow three rules: nothing conductive rides the changing field (titanium sled chassis, PEEK formers), nothing near the track is ferromagnetic except by design (BeCu springs, A-286 fasteners), and nothing outgasses near customer optics (E595-compliant polymers, dry films, sealed roller cartridges).
...
At the rated point (3U reference, 9.445~kg sled measured from the CAD solids, 1.3~m acceleration): exit velocity \textbf{16.5~m/s} at \textbf{10.7~g}, pulse 157~ms, peak current 347~A, bank state\-of\-charge sag 5.4\%, with roughly 4\% additional resistive dip at peak current (Fig.~\ref{fig:shot}). Energy drawn is 2.88~kJ per shot, with copper heat of 828~J and a further 86~J dissipated in the bank's own series resistance. Regenerative braking returns 297~J after release, so the net draw is 2.58~kJ. The payload receives 547~J, a \textbf{21.2\% electrical\-to\-payload efficiency}; 64\% of gross drawn energy becomes kinetic energy overall, of which the sled's share is now split between 297~J recovered and 952~J dissipated in the arrest brake. Efficiency falls with the heavier sled for two compounding reasons: a larger fraction of the same mechanical work goes into a mass that is then braked away, and the longer 157~ms pulse accrues more copper loss at unchanged current density. Recovery is what partially offsets the first of those, and it is bounded by geometry rather than by loss: over the available 240~mm the braking pulse lasts 15.6~ms and burns 15~J of copper, against 828~J over the 157~ms shot, so recovered energy rises monotonically with braking force up to the winding's rating and with distance beyond it. A 500~mm section would return 581~J and a 1~m section would arrest the sled outright, but the closed envelope already exceeds its target class by 44\% (Sec.~\ref{sec:lim}), so distance is not available here. Peak current during recovery is 225~A, below the shot's own 347~A, so the drive is not re-rated. The closed-loop Monte Carlo returns a dispersion of \textbf{0.027~m/s (3$\sigma$)} at a 16.2~m/s fleet setpoint (Fig.~\ref{fig:mc}), dominated by the sensing chain rather than plant tolerances. The setpoint must sit below the open-loop ceiling for the servo to retain authority; at 98.2\% of it, the headroom is the same fraction the superseded 20.0~m/s setpoint held. Table~\ref{tab:family} gives the payload family: the system is force-limited above 1U, so heavier payloads trade velocity, never structural margin, and the deployer mass each customer pays for varies by a factor of thirty across the family (Sec.~\ref{sec:family}).
...
\begin{table}[t]
\caption{Payload family at the rated point (1.3 m acceleration, 9.445 kg measured sled). Deployer mass per satellite uses the 76.9 kg dry mass and a packing model calibrated so the 3U case returns the twelve the magazine is laid out for}
\label{tab:family}
\centering\footnotesize
\begin{tabular}{lccrr}
\toprule
...
3U (4 kg) & 16.5 m/s & 10.7 g & 12 & \textbf{6.41}\\
...
The column that does move is deployer mass per satellite, by a factor of thirty. At 3U the machine spends \textbf{6.41~kg of deployer per customer}, which loses to a cold-gas propulsion module delivering the same 16.5~m/s at 0.5--1.2~kg by roughly a factor of eight. At PocketQube it spends \textbf{0.24~kg} and wins by two to five. The comparison inverts entirely on payload class, and on no machine parameter at all: not the thrust constant, not the stroke, not the sheet current.
...
Thermally the deployer is a pulsed, low-duty machine: each shot deposits heat in milliseconds, and the following minutes-to-orbit reject it. The per-shot budget is 843~J in the stator copper (828~J over the shot, 15~J over the regenerative braking pulse), 952~J in the brake fin, 113~J in the converter, and 94~J in the supercapacitor equivalent series resistance. Over a full twelve-shot campaign the total is 24.4~kJ; deposited into the roughly 13.5~kJ/K thermal mass of the structure this is a 1.8~K bulk rise, so no shot ever sees a meaningfully preheated machine and the adiabatic per-shot rises quoted earlier (0.35~K coil, 2.9~K fin) are the design drivers rather than steady state. The brake fin is the hottest element; its 2.9~K per-shot transient decays by radiation and conduction before the next shot at nominal 10--20~s cadence, so the fin never approaches the 35~K bound that a full twelve-shot campaign would reach only if it never radiated between shots. Recovering a quarter of the sled's energy is worth more thermally than electrically: it is the brake fin, not the bank, that the reduction relieves. Steady rejection is undemanding: the campaign-average dissipation is a few watts, and even the instantaneous 130~W burst, radiated over the following orbit from a 50$^\circ$C surface, needs only 0.32~m$^2$ of radiator, met by the structure's external skin. The orbital environment sets the endpoints: a hot-case beta-angle attitude with full solar and albedo input, and a cold-case eclipse, bound the supercapacitor and converter baseplates, both of which are specified to survive $-40$ to $+60^\circ$C with survival heaters covering the cold case at a few watts. The N45SH grade is selected precisely so the magnets tolerate the hot case without irreversible loss.
